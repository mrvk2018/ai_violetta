...
class ViolettaCommandService {
  ...
  Future<ViolettaIncomingSttResult> processIncomingSTT(
    ...
    final ViolettaAppLocale activeLocale =
        locale?.locale ?? ViolettaAppLocale.russian;
    String? speech;

    if (LocalUtilityFormatter.matchesExactDateQuery(normalized)) {
      speech = LocalUtilityFormatter.formatDate(now, activeLocale);
    } else if (LocalUtilityFormatter.matchesExactTimeQuery(normalized)) {
  ...
  Future<bool> executeOpenApp(String packageName) async {
    ...
    if (!violettaHasNativeAndroidBridge) {
      debugPrint(
        '[system_control] openApp package=$normalizedPackage '
  ...
  Future<void> executeCreateAlarm(int hour, int minutes) async {
    if (!violettaHasNativeAndroidBridge) {
      debugPrint(
        '[system_control] createAlarm hour=$hour minutes=$minutes '
        '(stub: ${defaultTargetPlatform.name})',
      );
      return;
    }
